...
import { resolveHtmlPath, parseCookies } from './util';
...
app.on('ready', () => {
  ...
  // Прослушивание события 'response' для получения кук
  currentSession.webRequest.onCompleted((details) => {
    const cookies = details.responseHeaders?.['Set-Cookie'];

    if (cookies) {
      // Установка каждой полученной куки
      cookies.forEach((cookie) => {
        const parsedCookies = parseCookies(cookie);

        currentSession.cookies.set(parsedCookies);
      });
    }
  });
  if (mainWindow) mainWindow.loadURL('http://localhost:1212');
});
